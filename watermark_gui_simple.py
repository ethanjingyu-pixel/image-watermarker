#!/usr/bin/env python3
"""
Image Watermarker GUI Application (Simplified Version)
...
class WatermarkGUI:
    ...
    def create_left_panel(self, parent):
        ...
        ttk.Button(btn_frame, text="清空列表", command=self.clear_images).pack(side=tk.RIGHT)
    ...
    def create_right_panel(self, parent):
        ...
        ttk.Radiobutton(format_frame, text="JPEG", variable=self.format_var, value="JPEG").pack(side=tk.LEFT, padx=(10, 5))
    ...
    def save_template(self):
        """保存水印模板"""
        template_name = tkinter.simpledialog.askstring("保存模板", "请输入模板名称:")
        if template_name:
            try:
